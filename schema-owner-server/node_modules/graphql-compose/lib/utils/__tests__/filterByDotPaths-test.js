...
var _filterByDotPaths = _interopRequireWildcard(require("../filterByDotPaths"));
...
describe('filterByDotPaths', () => {
  describe('filterByDotPaths()', () => {
    ...
    it('returns only requested field by string dot path', () => {
      expect((0, _filterByDotPaths.default)(data, 'source.age')).toEqual({
        'source.age': 30
      });
      expect((0, _filterByDotPaths.default)(data, 'args.data.1.id')).toEqual({
        'args.data.1.id': 2
      });
    });
    it('returns only requested fields by array of dot paths', () => {
      expect((0, _filterByDotPaths.default)(data, ['source.age', 'args.data.1.id', 'source.name'])).toEqual({
        'source.age': 30,
        'args.data.1.id': 2,
        'source.name': 'nodkz'
      });
    ...
    it('returns undefined value for non-existed path', () => {
      expect((0, _filterByDotPaths.default)(data, 'unexisted.path')).toEqual({
        'unexisted.path': undefined
      });
    });
    it('should pass object unchanged if filter is empty', () => {
      expect((0, _filterByDotPaths.default)(data, null)).toEqual(data);
    });
    it('returns fields and hide which in opts.hideFields', () => {
      const filteredData = (0, _filterByDotPaths.default)(data, null, {
        hideFields: {
