...
describe('TypeComposer', () => {
  ...
  beforeEach(() => {
    ...
    tc = new _.TypeComposer(objectType);
  });
  describe('fields manipulation', () => {
    it('getFields()', () => {
      ...
      const tc2 = _.TypeComposer.create('SomeType');
  ...
  describe('create() [static method]', () => {
    it('should create TC by typeName as a string', () => {
      const myTC = _.TypeComposer.create('TypeStub');

      expect(myTC).toBeInstanceOf(_.TypeComposer);
    ...
    it('should create TC by type template string', () => {
      const myTC = _.TypeComposer.create(`
        type TestTypeTpl {
          f1: String
    ...
    it('should create TC by GraphQLObjectType', () => {
      ...
      const myTC = _.TypeComposer.create(objType);
    ...
    it('should create type and store it in schemaComposer', () => {
      const SomeUserTC = _.TypeComposer.create('SomeUser');

      expect(_.schemaComposer.getTC('SomeUser')).toBe(SomeUserTC);
    });
    it('should create type and NOTE store root types in schemaComposer', () => {
      _.TypeComposer.create('Query');

      expect(_.schemaComposer.has('Query')).toBeFalsy();

      _.TypeComposer.create('Mutation');

      expect(_.schemaComposer.has('Query')).toBeFalsy();

      _.TypeComposer.create('Subscription');

      expect(_.schemaComposer.has('Query')).toBeFalsy();
    });
    it('createTemp() should not store type in schemaComposer', () => {
      _.TypeComposer.createTemp('SomeUser');
  ...
  describe('get()', () => {
    it('should return type by path', () => {
      const myTC = new _.TypeComposer(new _graphql.GraphQLObjectType({
        name: 'Readable',
        fields: {
          field1: {
            type: _graphql.GraphQLString,
  ...
  describe('Resolvers manipulation', () => {
    ...
    it('addResolver() should accept Resolver options and create instance', () => {
      ...
      expect(tc.getResolver('myResolver2')).toBeInstanceOf(_.Resolver);
  ...
  describe('addRelation()', () => {
    ...
    beforeEach(() => {
      ...
      ArticleTC = _.TypeComposer.create(`
        type Article {
          id: Int,
          userId: Int,
          title: String,
  ...
  it('should have chainable methods', () => {
    ...
    expect(tc.setResolver('myResolver', new _.Resolver({
      name: 'myResolver'
    }))).toBe(tc);
    expect(tc.addResolver(new _.Resolver({
      name: 'myResolver'
    }))).toBe(tc);
  ...
  describe('getFieldTC()', () => {
    const myTC = _.TypeComposer.create('MyCustomType');

    myTC.addFields({
      scalar: 'String',
      list: '[Int]',
      obj: _.TypeComposer.create(`type MyCustomObjType { name: String }`),
      objArr: [_.TypeComposer.create(`type MyCustomObjType2 { name: String }`)]
    });
