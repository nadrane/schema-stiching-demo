...
import { GraphQLObjectType, GraphQLList, GraphQLNonNull, GraphQLInputObjectType, getNamedType, GraphQLInterfaceType } from './graphql';
...
import { typeByPath } from './utils/typeByPath'; // import { deprecate } from './utils/debug';
...
export class TypeComposer {
  ...
  static createTemp(opts) {
    ...
    if (isString(opts)) {
      ...
      if (NAME_RX.test(typeName)) {
        TC = new this.schemaComposer.TypeComposer(new GraphQLObjectType({
          name: typeName,
          fields: () => ({})
        }));
      } else {
      ...
      const type = new GraphQLObjectType(_objectSpread({}, opts, {
    ...
    Object.keys(newFields).forEach(fieldName => {
      ...
      if (names.length === 0) {
        ...
        if (!this.hasField(name)) {
          childTC = this.schemaComposer.TypeComposer.createTemp(`${this.getTypeName()}${upperFirst(name)}`);
          this.setField(name, {
            type: childTC,
            resolve: () => ({})
          });
  ...
  clone(newTypeName) {
    ...
    const cloned = new this.schemaComposer.TypeComposer(new GraphQLObjectType({
      name: newTypeName,
      fields: newFields
    }));
